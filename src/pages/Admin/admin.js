...
const Admin = () => {
  ...
  const [changeAgentEmail, setChangeAgentEmail] = useState('');
  const [assignModalVisible, setAssignModalVisible] = useState(false);
  const [changeAgentModalVisible, setChangeAgentModalVisible] = useState(false);
  const [requestId] = useState('');
  const [serviceTypeFilter, setServiceTypeFilter] = useState('');
  const [statusFilter, setStatusFilter] = useState('');
  const [priorityFilter, setPriorityFilter] = useState('');
  const [ setActiveFilterDropdown] = useState(false);
  ...
  const handleAssignRequest = async (requestId) => {
    try {
      await axios.put(`https://sathservicedesk-sathservicedesk-staging-dpdaa2a3dkd7e7as.eastus2-01.azurewebsites.net/${requestId}`, {
        agent_email: assignEmail
      }, {
        headers: {
          'Content-Type': 'application/json',
          'userEmail': userEmail,
      ...
      setFeedbackMessage('The request has been successfully assigned.');
      setFeedbackType('success');
    } catch (err) {
      setFeedbackMessage('An error occurred while assigning the request. Please try again.');
  ...
  const handleChangeRequestAgent = async (requestId) => {
    try {
      await axios.put(`https://sathservicedesk-sathservicedesk-staging-dpdaa2a3dkd7e7as.eastus2-01.azurewebsites.net/change-request-agent/${requestId}`, {
        agent_email: changeAgentEmail
      }, {
        headers: {
          'Content-Type': 'application/json',
          'userEmail': userEmail,
      ...
      alert('Request agent changed successfully');
      setChangeAgentModalVisible(false);
      setChangeAgentEmail('');
    } catch (err) {
      alert('Failed to change request agent');
    }
  };

  const handleViewDetails = async (requestId) => {
    try {
      const response = await axios.get(`https://sathservicedesk-sathservicedesk-staging-dpdaa2a3dkd7e7as.eastus2-01.azurewebsites.net/get-request/${requestId}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer your-auth-token',
          'userEmail': userEmail,
              ...
              <tbody>
                {filteredRequests.map(request => (
                  <tr key={request.id}>
                    ...
                    <div>
                    <td>
                      <button onClick={() => handleViewDetails(request.id)} className="view-details-btn"><FaEye className="eye-icon" /></button>
                    </td>
                    <td>
                      <button onClick={() => handleCloseRequest(request.id)} className="close-request-btn"> <FaTimes className="eye-icon" /></button>
                    </td>
                    </div>
                  </tr>
                ))}
...
{selectedRequest && (
  <div className="modal-overlay">
    <div className="details-modal-content">
      <button className="details-modal-close-btn" onClick={() => setSelectedRequest(null)}>×</button>
      <h2>Request Details</h2>
      <table>
          <tbody>
            <tr>
              <td>ID:</td>
              <td>MNTRQ00{selectedRequest.id}</td>
            </tr>
            <tr>
              <td>Category:</td>
            ...
            <tr>
              ...
              <td>{selectedRequest.requester_name}</td>
            </tr>
            <tr>
              <td>Requester Email:</td>
              <td>{selectedRequest.requester_email}</td>
            </tr>
            <tr>
              <td>Created Date:</td>
              <td>{selectedRequest.created_date}</td>
            </tr>
            <tr>
              <td>Due Date:</td>
              <td>{selectedRequest.due_date}</td>
            </tr>
            <tr>
              <td>Priority:</td>
            ...
            <tr>
              <td>Technician:</td>
              <td>{selectedRequest.technician}</td>
            </tr>
            <tr>
              <td>Updated Date:</td>
              <td>{selectedRequest.updated_date}</td>
            </tr>
          </tbody>
        </table>
        <div className="button-container">
  <button onClick={() => setShowCreateTaskModal(true)} className="details-modal-btn">Create Task</button>
  <button onClick={() => setAssignModalVisible(true)} className="details-modal-btn">Assign Technician</button>
  <button onClick={() => setChangeAgentModalVisible(true)} className="details-modal-btn">Change Agent</button>
</div>
...
{assignModalVisible && (
  <div className="modal-overlay">
    <div className="details-modal-content">
      ...
      <button className="details-modal-close-btn" onClick={() => setAssignModalVisible(false)}>×</button>
      ...
      />
      <button onClick={() => handleAssignRequest(selectedRequest.id)} className="details-modal-btn">Assign</button>
    </div>
  </div>
)}
...
{changeAgentModalVisible && (
  <div className="modal-overlay">
    <div className="details-modal-content">
      <h3>Change Agent</h3>
      <button className="details-modal-close-btn" onClick={() => setChangeAgentModalVisible(false)}>×</button>
      <input
        type="email"
        value={changeAgentEmail}
        onChange={(e) => setChangeAgentEmail(e.target.value)}
        placeholder="Enter new agent email"
      />
      <button onClick={() => handleChangeRequestAgent(selectedRequest.id)} className="details-modal-btn">Change Agent</button>
    </div>
  </div>
)}
